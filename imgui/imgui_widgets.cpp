...
bool ImGui::Checkbox2(const char* label, bool* v)
{
    ...
    ImU32 check_col = GetColorU32(ImGuiCol_CheckMark);
    bool mixed_value = (window->DC.LastItemStatusFlags & ImGuiItemFlags_MixedValue) != 0;
    const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
    ...
    if (*v)
    {
        window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + pad, check_bb.Min.y + pad), ImVec2(check_bb.Max.x - pad, check_bb.Max.y - pad), ImColor(0, 0, 255, int(t * 205)), 2);
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id, bool is_contents_visible, bool* out_just_closed, bool* out_text_clipped)
{
    ...
    if (out_text_clipped)
    {
        *out_text_clipped = (text_ellipsis_clip_bb.Min.x + label_size.x) > text_pixel_clip_bb.Max.x;
        //draw_list->AddCircle(text_ellipsis_clip_bb.Min, 3.0f, *out_text_clipped ? IM_COL32(0, 0, 255, 255) : IM_COL32(0, 255, 0, 255));
    }
